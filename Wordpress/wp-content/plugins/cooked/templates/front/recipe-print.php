...
function cooked_print_enqueues(){
	$min = ( COOKED_DEV ? '' : '.min' );
	wp_enqueue_style( 'cooked-print', COOKED_URL . 'assets/css/print' . esc_attr( $min ) . '.css', array(), COOKED_VERSION, 'screen,print' );
	wp_enqueue_style( 'cooked-icons', COOKED_URL . 'assets/css/icons' . esc_attr( $min ) . '.css', array(), COOKED_VERSION, 'screen,print' );
