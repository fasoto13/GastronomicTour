        	...
        	foreach($recipe_settings as $key => $val):
        		if (!is_array($val)):
        			...
        			if ( $key === "content" || $key === "excerpt" ):
        				...
        				$_recipe_settings[$key] = sanitize_text_field( $val );
        			endif;

        		else:
        			foreach($val as $subkey => $subval):
        				if (!is_array($subval)):
		        			$_recipe_settings[$key][$subkey] = sanitize_text_field($subval);
		        		else:
		        			foreach( $subval as $sub_subkey => $sub_subval ):
		        				if ( !is_array($sub_subval) ):
		        					if ( $sub_subkey == 'content' || $key == 'ingredients' && $sub_subkey == 'name' || $key == 'ingredients' && $sub_subkey == 'section_heading_name' || $key == 'directions' && $sub_subkey == 'section_heading_name' ):
				        				...
				        				$_recipe_settings[$key][$subkey][$sub_subkey] = sanitize_text_field( $sub_subval );
				        			endif;
				        		else:
				        			foreach($sub_subval as $sub_sub_subkey => $sub_sub_subval):
				        				if (!is_array($sub_sub_subval)):
						        			$_recipe_settings[$key][$subkey][$sub_subkey][$sub_sub_subkey] = sanitize_text_field($sub_sub_subval);
						        		else:
						        			foreach($sub_sub_subval as $sub_sub_sub_subkey => $sub_sub_sub_subval):
						        				$_recipe_settings[$key][$subkey][$sub_subkey][$sub_sub_subkey][$sub_sub_sub_subkey] = sanitize_text_field($sub_sub_sub_subval);
						        			endforeach;
						        		endif;
				        			endforeach;
				        		endif;
