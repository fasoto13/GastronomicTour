...
class Cooked_Shortcodes {
	...
	public function cooked_timer( $atts, $content = null ){
		...
		return '<span class="cooked-timer"><a data-timer-id="' . esc_attr( $timer_id ) . '" data-seconds="' . esc_attr( $seconds ) . '" data-desc="' . ( $desc ? esc_attr( $desc ) : esc_attr( $content ) ) . '"><i class="cooked-icon cooked-icon-clock"></i> ' . wp_kses_post( $content ) . '</a></span>';
	...
	public function cooked_recipe_card_shortcode( $atts, $content = null ){
		...
		$hide_image = sanitize_text_field( $atts['hide_image'] );
		$hide_total = sanitize_text_field( $atts['hide_total'] );
		$hide_title = sanitize_text_field( $atts['hide_title'] );
		$hide_excerpt = sanitize_text_field( $atts['hide_excerpt'] );
		$hide_author = sanitize_text_field( $atts['hide_author'] );
	...
	public function cooked_categories_shortcode( $atts, $content = null ){
		...
		$hide_empty = sanitize_text_field( $atts['hide_empty'] );
	...
	public function cooked_recipe_list_shortcode( $atts, $content = null ){
		...
		$recipes = sanitize_text_field( $atts['recipes'] );
	...
	public function cooked_recipe_shortcode( $atts, $content = null ){
		...
		if ( $recipe_id ):
			...
			if ( !$recipe_settings || $recipe_settings && empty( $recipe_settings ) ):
				return wpautop( '<strong>[cooked-recipe id="'.intval( $recipe_id ).'"]</strong><br><em>' . esc_html__( '(recipe not found or in draft status)', 'cooked' ) . '</em>' );
			else:
						...
						$gallery_html = '<div class="cooked-recipe-gallery"' . ( !empty($gallery_options_array) ? ' ' . implode( ' ', $gallery_options_array ) : '' ) . '>';
							...
							if ( isset($gallery_items) && !empty($gallery_items) ):
								...
								foreach( $gallery_items as $item ):
									...
									$gallery_html .= '<a href="' . esc_url( $image_src[0] ) . '" data-caption="'.esc_attr( $image_title ).'"><img src="' . wp_get_attachment_image_url( $item, 'thumbnail' ) . '" /></a>';
								endforeach;
	...
	public function cooked_info_shortcode($atts, $content = null) {
		...
		$left = ( $atts['left'] ? array_map( 'trim', explode( ',', sanitize_text_field($atts['left']) ) ) : false );
		$right = ( $atts['right'] ? array_map( 'trim', explode( ',', sanitize_text_field($atts['right']) ) ) : false );
		$include = ( $atts['include'] ? array_map( 'trim', explode( ',', sanitize_text_field($atts['include']) ) ) : false );
		$exclude = ( $atts['exclude'] ? array_map( 'trim', explode( ',', sanitize_text_field($atts['exclude']) ) ) : false );
	...
	public static function cooked_info_author(){
		...
		if (in_array('author',$_cooked_settings['recipe_info_display_options'])):
			...
			echo '<span class="cooked-author' . ( $hide_avatars ? ' cooked-no-avatar' : '' ) . '">';
				...
				echo '<strong class="cooked-meta-title">' . esc_html__('Author','cooked') . '</strong>' . ( $clickable && $permalink ? '<a href="' . esc_url( $permalink ) . '">' : '' ) . esc_html( $author['name'] ) . ( $clickable && $permalink ? '</a>' : '' );
			echo '</span>';
	...
	public function cooked_ingredients_shortcode($atts, $content = null) {
		...
		$checkboxes = sanitize_text_field($atts['checkboxes']);
	...
	public function cooked_directions_shortcode($atts, $content = null) {
		...
		$step = 0;
		$numbers = sanitize_text_field( $atts['numbers'] );
